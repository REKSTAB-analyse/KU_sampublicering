...
Cirklens størrelse afspejler antallet af unikke forfatterbidrag: jo større cirkel, jo flere forfatterbidragbidrag. 
...
- Er samarbejedet koncentreret omkring få enheder - eller relativt bredt fordelt? 
...
**Tip:** Klik på en cirkel for at se dens detaljer og nærmeste samarbedspartnere i panelet i højre hjørne.
""")
...
"""
    ...
    if org_ew_all is not None:
        ...
        _active_ew = org_ew_all if _use_all else org_ew
    else:
        _active_ew = org_ew

    grand_tot = sum(org_tot.values()) or 1
    grand_ew  = sum(_active_ew.values())  or 1

    rows = []
    for u in units:
        pct_tot = 100 * org_tot.get(u, 0) / grand_tot
        pct_ew  = 100 * _active_ew.get(u, 0)  / grand_ew
        rows.append({
            "unit":    u,
    ...
    # ── Scatter: forfatterbidrag% (x) vs forfatterpar% (y) ───────────────────────────
    ...
    _fig_sc.update_layout(
        ...
        showlegend=False,
        title=dict(text=_full_title) if _full_title else None,
    )
    ...
    # ── Diverging bar: difference (forfatterpar% − bidrag%) ──────────────────────────
    ...
    _fig_div.update_layout(
        ...
        margin=dict(l=140, r=80, t=50 if _div_title else 10),
        title=dict(text=_div_title) if _div_title else None,
    )
    ...
    _tbl_rows = [
        {org_label: r["unit"],
         ...
         "Samlet forfatterpar-andel (%)":   r["pct_ew"],
         "Forskel (pp)":     r["diff"]}
    ...
    _tbl_schema = [
        ...
        ("Samlet forfatterpar-andel (%)",    pa.float64()),
        ("Forskel (pp)",      pa.float64()),
    ]

    if show_table:
        _tbl_col = f"Forfatterpar-andel (%){_title_suffix}"
        _tbl_rows_final = [{**r, _tbl_col: r.pop("Samlet forfatterpar-andel (%)")} for r in _tbl_rows]
        _tbl_schema_final = [
            (org_label,          pa.string()),
            ("Bidrag-andel (%)", pa.float64()),
            (_tbl_col,           pa.float64()),
        ...
        with st.expander("Se tabel"):
            st.dataframe(build_table(_tbl_rows_final, _tbl_schema_final), hide_index=True, width="stretch")
            st.download_button(
                "Download (.xlsx)",
                data=rows_to_excel_bytes(_tbl_rows_final, [n for n, _ in _tbl_schema_final]),
...
**Forfatterpar** tæller hver unik kombination af to forfattere på én publikation - en artikel med fire forfattere
giver altså seks forfatterpar. 
**Forfatterbidrag** er summen af unikke forfattere i sampublikationer.
...
""")
    ...
    st.markdown(
f"""**{intra_label.capitalize()}-forfatterpar** udgør **{int(_intra_pubs_plot)}** (**{_pct_intra_plot}%**),
**{inter_label.capitalize()} forfatterpar** udgør **{int(_inter_pubs_plot)}** (**{_pct_inter_plot}%**)."""
+ (f" På **institutniveau** er intra-andelen **{_pct_intra_inst_plot}%**."
   if base_mode(mode) in ("FI", "FIG") else "")
+ (f" På **stillingsgruppenivu** er intra-andelen **{_pct_intra_grp_plot}%**."
   if base_mode(mode) in {m.replace("S","") for m in _grp_modes} | _grp_modes else ""))
...
+ _ew_top_txt if _ew_top_txt else "")
    ...
    st.markdown(
f"""
#### Fakulteternes rækkevidde i {year}

**Rækkevidde** måler, hvor mange *forskellige* fakulteter et fakultet samarbejder med - uanset hvor meget 
de samarbejder. Et fakultet kan have mange forfatterpar, men hvis de primært er koncentreres om én eller 
to partnere, er rækkevidden lav. En bred rækkevidde indikerer et diverst samarbejdsmønster på tværs af KU. 
""")

    _fac_partners: dict[str, set] = {}
    for u, v, w, *_ in edges_keep:
        fac_u = node_meta.get(u, {}).get("fac", "")
        fac_v = node_meta.get(v, {}).get("fac", "")
        if fac_u and fac_v and fac_u != fac_v:
            _fac_partners.setdefault(fac_u, set()).add(fac_v)
            _fac_partners.setdefault(fac_v, set()).add(fac_u)

    _all_facs = {
        node_meta.get(nid, {}).get("fac", "")
        for nid in node_meta
        if node_meta.get(nid, {}).get("fac")
    }
    _max_reach = len(_all_facs) - 1

    _reach_sorted = [(f, len(_fac_partners.get(f, set()))) for f in _facs_ord]

    if any(n > 0 for _, n in _reach_sorted):
        _reach_facs   = [f for f, _ in _reach_sorted]
        _reach_vals   = [n for _, n in _reach_sorted]
        _reach_colors = [faculty_base_colors.get(f, "#122947") if faculty_base_colors else "#122947" for f in _reach_facs]

        # Analytisk tekst
        _reach_top, _reach_top_n = _reach_sorted[0]
        _reach_bot, _reach_bot_n = _reach_sorted[-1]
        
        _fig_reach = go.Figure(go.Bar(
            y=_reach_facs, x=_reach_vals,
            orientation="h",
            marker_color=_reach_colors,
            text=[f"{v} / {_max_reach}" for v in _reach_vals],
            textposition="inside",
        ))
        _fig_reach.update_layout(
            xaxis=dict(title="Antal samarbejdsfakulteter", range=[0, _max_reach + 0.5], dtick=1),
            height=max(300, 50 * len(_reach_facs)),
            margin=dict(l=80, t=50, r=80),
            yaxis=dict(autorange="reversed"),
            title = f"Fakulteternes rækkevidde, {year}"
        )
        st.plotly_chart(_fig_reach, width="stretch", key=f"fac_reach_{year}_{mode}",
        config={
            "toImageButtonOptions": {
                "format": "png",
                "scale": 3,
...
""")
    ...
    _tab_yr_tot, _tab_yr_ew, _tab_yr_reach = st.tabs(["Forfatterbidrag per fakultet", "Forfatterpar per fakultet", "Fakulteternes rækkevidde"])
    ...
    with _tab_yr_reach: 
        _facs_reach_in_data = sorted({
            f
            for s in (all_years_data or {}).values()
            for f in s.get("fac_reach", {})
        })
        _render_year_comparison(
            all_years_data,
            series=[(fac, "fac_reach", fac) for fac in _facs_reach_in_data],
            title=f"Udvikling i fakulteternes ræddevidde, {years_sorted_fac[0]}-{years_sorted_fac[-1]}",
            yaxis_label="Antal samarbejdspartnere",
            colors=faculty_base_colors,
            key_suffix=f"fac_reach_{year}_{mode}",
            show_table=True,
        )
...
""")
    ...
    if _inst_ew_filt:
        ...
        if all_years_data and year in all_years_data:
            ...
            _ix = all_years_data[year].get("inst_inter_ew_all", {})
            _inst_ew_all_for_share = {
                i: _ii.get(i, 0.0) + _ix.get(i, 0.0)
                for i in set(_ii) | set(_ix)
                if _fac_ok(i)
            }
        _render_share_comparison(
            _inst_tot_filt,
...
def render_pos_chart(pos_counts: dict, tab_key: str, year: int, mode: str, grp_tot_size: dict = None):
    ...
    _tab_abs, _tab_pct, _tab_rate = st.tabs(["Antal", "Andel (%)", "Rate (per forsker)"])
    ...
    with _tab_rate:
        st.markdown("Antal gange en typisk forsker i gruppen optræder i hver forfatterposition - beregnet som antal positioner divideret med antal unikke forfatterbidrag i gruppen.")
        if not grp_tot_size:
...
den lokale kønsfordelingen ville forudsige - såkalt **kønshomofili**. 

Opgørelsen er lavet på **individniveau**: hvert forfatterpar tælles fra begge forfatters perspektiv.
...
at publicere med eget køn. Indekset bør derfor tolkes som et mål for *konssegregation i sampubliceringsmønstre* - 
...
""")
    ...
    def _org_from_node_id(nid: str) -> str:
        parts = nid.split("|")
        keys = []
        if "Fakulteter"       in _selected_levels: keys.append(parts[0] if len(parts) > 0 else "")
        if "Institutter"      in _selected_levels: keys.append(parts[1] if len(parts) > 1 else "")
        if "Stillingsgrupper" in _selected_levels: keys.append(parts[2] if len(parts) > 2 else "")
        return " - ".join(k for k in keys if k)

    def _node_id_ok(nid: str) -> bool:
        parts = nid.split("|")
        if len(parts) < 3:
            return True
        if _køn_fac_filter  and parts[0] not in _køn_fac_filter:  return False
        if _køn_inst_filter and parts[1] not in _køn_inst_filter: return False
        if _køn_grp_filter  and parts[2] not in _køn_grp_filter:  return False
        return True

    org_sex_ew2: dict[str, dict[str, float]] = defaultdict(lambda: defaultdict(float))
    for edge in _edges_f:
        u, v, w = edge[0], edge[1], edge[2]
    ...
    for raw_key, counts in (sex_pairs or {}).items():
        ...
        if _køn_inst_filter and inst not in _køn_inst_filter: continue
        if _køn_grp_filter  and grp  not in _køn_grp_filter:  continue
        org = _org_key({"fac": fac, "inst": inst, "grp": grp})
        total = counts.get("same", 0) + counts.get("cross", 0)
        if org in sex_size:
            org_sex_ew_all[org][focal_sex] = org_sex_ew_all[org].get(focal_sex, 0.0) + total
    ...
    for raw_key, counts in (sex_pairs or {}).items():
        ...
        if _køn_grp_filter  and grp  not in _køn_grp_filter:  continue
        total = counts.get("same", 0) + counts.get("cross", 0)
        sex_ew_all[focal_sex] = sex_ew_all.get(focal_sex, 0.0) + total
...
""")
    ...
    _intl_fb = {org: sum(v for k, v in cs_size.get(org, {}).items() if k != "DK")
                for org in orgs_for_comparison}
    _render_share_comparison(
        _intl_fb,
